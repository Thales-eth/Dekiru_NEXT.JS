    ...
    return (
        <div className={styles.classCard}>
            <div className={styles.icons}>
                {
                    user.id === singleClass.teacher._id
                        ?
                        <>
                            <CloseButton handleClick={() => deleteClass(singleClass._id)} size={50} />
                            <Link href={""} className={styles.editIcon} onClick={triggerActions}><AiFillEdit size={50} className={styles.editBtn} /></Link>
                        </>
                        :
            ...
            <div className={styles.info}>
                <Link href={""} onClick={triggerClassNavigation}><p className={styles.title}>{singleClass?.title}</p></Link>
